...
func init() {
	...
	lnn.RegisterCalibrator("arc-phimini35", func(cfg lnn.Config) (lnn.Calibrator, error) {
		return NewPhiMiniCalibrator(cfg)
	})

	// Also register with common model name variants
	lnn.RegisterCalibrator("arc-phi", func(cfg lnn.Config) (lnn.Calibrator, error) {
		return NewPhiMiniCalibrator(cfg)
	})

	lnn.RegisterCalibrator("arc-phimini", func(cfg lnn.Config) (lnn.Calibrator, error) {
		return NewPhiMiniCalibrator(cfg)
	})
}
