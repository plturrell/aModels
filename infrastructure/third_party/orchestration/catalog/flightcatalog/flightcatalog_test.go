...
func TestFetch(t *testing.T) {
	...
	if suite.Name != "agentic" {
		t.Errorf("unexpected suite name %q", suite.Name)
	}
	if suite.ToolCount != 1 {
		t.Errorf("unexpected tool count %d", suite.ToolCount)
	}
	if suite.Implementation != "agentic-mcp" {
		t.Errorf("unexpected implementation %q", suite.Implementation)
	}
	if suite.Version != "0.1.0" {
		t.Errorf("unexpected version %q", suite.Version)
	}
	if suite.AttachedAt.IsZero() {
		t.Error("expected attached timestamp")
	}
...
func TestFetchMissingAddress(t *testing.T) {
