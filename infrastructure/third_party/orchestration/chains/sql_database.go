...
import (
	...
	"github.com/plturrell/agenticAiETH/agenticAiETH_layer4_Orchestration/tools/sqldatabase"
)
...
func (s SQLDatabaseChain) Call(ctx context.Context, inputs map[string]any, options ...ChainCallOption) (map[string]any, error) {
	...
	// Execute sql query
	queryRows, err := s.Database.Query(ctx, sqlQuery)
	if err != nil {
		return nil, err
	}

	// Convert query result to string
	queryResult, err := sqldatabase.QueryToString(queryRows)
	if err != nil {
		return nil, err
	}
