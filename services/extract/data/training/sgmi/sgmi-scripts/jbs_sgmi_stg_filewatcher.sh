...
v_filename=$(echo SGMI_MASTER_FILE_EXTRACT_"$v_workspace_id"_"$v_year""$v_month""$v_date".txt)
...
do
	echo "Loop Number $v_loop_counter at "`date` >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
       ...
       if [ "$3" == 'BCRS_PRA' ]
        then
	if [ -r $BCRS_LOCATION/PRA/SGMI_MASTER_FILE_EXTRACT_"$v_workspace_id"_"$v_year""$v_month""$v_date".txt ]
        then
        echo "PRA file available for " $v_workspace_id >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
        sleep 30
	sh $JBS_APPHOME/bin/jbs_sgmi_stg_ingestion.sh $1 $2 $3 $4 $5

       if [[ $? == 0 ]]
       then
       echo "PRA files Loaded successfully for " $v_workspace_id >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
        exit 0   
        else
        echo "Error with PRA file loading for " $v_workspace_id >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
        exit 1
       fi
        ...
        if [ "$3" == 'BCRS_ND' ]
        then
        if [ -r $BCRS_LOCATION/ND/SGMI_MASTER_FILE_EXTRACT_"$v_workspace_id"_"$v_year""$v_month""$v_date".txt ]
        then
        echo "ND file available for " $v_workspace_id >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
        sleep 30
	sh $JBS_APPHOME/bin/jbs_sgmi_stg_ingestion.sh $1 $2 $3 $4 $5

       if [[ $? == 0 ]]
       then
       echo "ND filed Loaded successfully for " $v_workspace_id >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
        exit 0   
        else
        echo "Error with ND file loading for " $v_workspace_id >> $JBS_LOG_DIR/File_Watcher_STG_"$3"_"$v_workspace_id"_"$DT".log
        exit 1
       fi
    ...
    if [ "$3" == 'BCRS_RC' ]
        then
        if [ -r $BCRS_LOCATION/RC/SGMI_MASTER_FILE_EXTRACT_"$v_workspace_id"_"$v_year""$v_month""$v_date".txt ]
        then
