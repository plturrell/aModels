...
func NewMurexIntegration(conn Connector, mapper ModelMapper, graphClient GraphClient, logger *log.Logger) *MurexIntegration {
...
func (mi *MurexIntegration) IngestRegulatoryCalculations(ctx context.Context, filters map[string]interface{}) error {
	...
	for _, record := range data {
		...
		if tradeID, ok := record["trade_id"].(string); ok {
			tradeNodeID := fmt.Sprintf("trade-%s", tradeID)
			edge := CreateTradeToCalculationEdge(tradeNodeID, calc.ID, map[string]interface{}{ "source_system": "Murex_FMRP" })
			edges = append(edges, *edge)
		}
	}
...
func (mi *MurexIntegration) DiscoverSchema(ctx context.Context) (*SourceSchema, error) {
