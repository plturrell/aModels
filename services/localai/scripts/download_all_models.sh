...
# Download 6 models for LocalAI
# 5 from Kaggle + 1 from GitHub (TinyRecursiveModels)
...
echo "📥 Downloading 6 models for LocalAI..."
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📦 Model 1/6: Gemma 2B-it (TensorRT-LLM)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📦 Model 2/6: Phi-3.5-mini-instruct (PyTorch)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📦 Model 3/6: IBM Granite 4.0-h-micro (Transformers)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📦 Model 4/6: Gemma 7B-it (TensorRT-LLM)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
if [ ! -f "$MODEL4_DIR/model.tar.gz" ] && [ ! -d "$MODEL4_DIR/7b-it" ]; then
    ...
    # Fallback to curl if kaggle CLI didn't work
    if [ ! -f "$MODEL4_DIR/model.tar.gz" ] && [ ! -d "$MODEL4_DIR/7b-it" ]; then
...
# Model 5: VaultGemma 1B Transformers
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📦 Model 5/6: VaultGemma 1B (Transformers)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
MODEL5_DIR="$MODELS_DIR/vaultgemma-1b-transformers"
mkdir -p "$MODEL5_DIR"

if [ ! -f "$MODEL5_DIR/model.tar.gz" ] && [ ! -f "$MODEL5_DIR/model.safetensors" ]; then
    echo "   Downloading from Kaggle..."
    curl -L -u "$KAGGLE_USERNAME:$KAGGLE_KEY" \
        -o "$MODEL5_DIR/model.tar.gz" \
        "https://www.kaggle.com/api/v1/models/google/vaultgemma/transformers/1b/1/download"
    
    if [ -f "$MODEL5_DIR/model.tar.gz" ]; then
        SIZE=$(du -h "$MODEL5_DIR/model.tar.gz" | cut -f1)
        echo "   ✅ Downloaded ($SIZE)"
        echo "   📦 Extracting..."
        cd "$MODEL5_DIR"
        tar -xzf model.tar.gz 2>/dev/null && echo "   ✅ Extracted" || echo "   ⚠️  Check extraction manually"
...
# Model 6: TinyRecursiveModels (GitHub)
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📦 Model 6/6: TinyRecursiveModels (GitHub)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
MODEL6_DIR="$MODELS_DIR/TinyRecursiveModels"
TRM_REPO="https://github.com/SamsungSAILMontreal/TinyRecursiveModels.git"

if [ ! -d "$MODEL6_DIR" ] || [ ! -d "$MODEL6_DIR/.git" ]; then
    echo "   Cloning from GitHub..."
    if command -v git &> /dev/null; then
        cd "$MODELS_DIR"
        git clone "$TRM_REPO" "$MODEL6_DIR" 2>&1 | grep -E "(Cloning|done|error)" || true
        
        if [ -d "$MODEL6_DIR/.git" ]; then
            echo "   ✅ Cloned successfully"
            echo "   📦 Installing dependencies..."
            cd "$MODEL6_DIR"
            if [ -f "requirements.txt" ]; then
                pip install -q -r requirements.txt 2>&1 | tail -3 || echo "   ⚠️  Check dependencies manually"
    ...
    cd "$MODEL6_DIR"
    git pull 2>&1 | grep -E "(Already|Updating|error)" || echo "   ✅ Updated"
fi

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "✅ All 6 models download complete!"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "📊 Summary:"
...
echo "   ✅ Model 5: VaultGemma 1B Transformers → $MODEL5_DIR"
echo "   ✅ Model 6: TinyRecursiveModels → $MODEL6_DIR"
echo ""
echo "📁 Model directories:"
for dir in "$MODEL1_DIR" "$MODEL2_DIR" "$MODEL3_DIR" "$MODEL4_DIR" "$MODEL5_DIR" "$MODEL6_DIR"; do
    if [ -d "$dir" ]; then
...
echo "   2. For TinyRecursiveModels: Review README for training/inference setup"
echo "   3. Update LocalAI config to use Kaggle models"
echo "   4. Restart LocalAI server: cd /home/aModels/services/localai && ./start-production.sh"
echo ""
echo "📚 TinyRecursiveModels info:"
echo "   - 7M parameter recursive reasoning model"
echo "   - Achieves 45% on ARC-AGI-1, 8% on ARC-AGI-2"
echo "   - GitHub: https://github.com/SamsungSAILMontreal/TinyRecursiveModels"
