...
wait_for_endpoint "${WAIT_FOR_ORCHESTRATOR:-}" 60
wait_for_endpoint "${WAIT_FOR_EXTRACT:-}" 60

FLOW_BIN="${ROOT_DIR}/cmd/flow-run/flow-run"

if [[ -x "${FLOW_BIN}" ]]; then
  if [[ -n "${EXTRA_ARGS:-}" ]]; then
    exec "${FLOW_BIN}" ${EXTRA_ARGS}
  else
    exec "${FLOW_BIN}"
  fi
fi
...
cd "${ROOT_DIR}"
if [[ -n "${EXTRA_ARGS:-}" ]]; then
  exec go run ./cmd/flow-run ${EXTRA_ARGS}
else
  exec go run ./cmd/flow-run
fi
