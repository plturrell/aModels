...
// RelationalJobProcessor handles async processing of relational table requests.
type RelationalJobProcessor struct {
...
// NewRelationalJobProcessor creates a new relational job processor.
func NewRelationalJobProcessor(tracker *RequestTracker, pipeline *RelationalPipeline, logger *log.Logger) *RelationalJobProcessor {
	processor := &RelationalJobProcessor{
		tracker:      tracker,
		pipeline:     pipeline,
...
func (jp *RelationalJobProcessor) SubmitJob(requestID string, query map[string]interface{}, webhookURL string) error {
	jp.mu.Lock()
	defer jp.mu.Unlock()
...
func (jp *RelationalJobProcessor) processJob(job *Job) {
	jp.mu.Lock()
	...
	// Process tables
	_, err := jp.pipeline.ProcessTablesWithTracking(job.Context, job.RequestID, job.Query)
...
func (jp *RelationalJobProcessor) CancelJob(requestID string) error {
	jp.mu.Lock()
	defer jp.mu.Unlock()
...
func (jp *RelationalJobProcessor) GetJob(requestID string) (*Job, bool) {
	jp.mu.RLock()
	defer jp.mu.RUnlock()
...
func (jp *RelationalJobProcessor) sendWebhookNotification(job *Job) {
...
func (jp *RelationalJobProcessor) processWebhooks() {
...
func (jp *RelationalJobProcessor) sendWebhook(notification WebhookNotification) {
...
func (jp *RelationalJobProcessor) Start() {
