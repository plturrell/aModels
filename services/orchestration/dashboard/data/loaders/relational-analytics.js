/**
 * Data loader for relational analytics data
...
export default async function() {
  ...
  const url = `${apiBase}/api/relational/history?limit=100`;
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch relational history: ${response.status} ${response.statusText}`);
    }
    ...
    console.error("Error loading relational analytics:", error);
    return {
      error: true,
      message: error.message || "Failed to load relational analytics",
      requests: [],
      trends: {}
    };
  }
