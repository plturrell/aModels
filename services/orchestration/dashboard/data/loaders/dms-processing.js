/**
 * Data loader for DMS processing status
 * Fetches processing request status from DMS API
 */
...
export default async function(requestId) {
  ...
  const url = `${apiBase}/api/dms/status/${requestId}`;
  ...
  try {
    ...
    console.error("Error loading DMS processing status:", error);
