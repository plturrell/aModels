---
title: DMS Processing Dashboard
description: Real-time processing status, progress, and statistics
---

```js
import * as Plot from "@observablehq/plot";
import dmsProcessingStatus from "../data/loaders/dms-processing.js";
import {html} from "@observablehq/stdlib";
import {emptyStateNoRequest, emptyStateLoading, emptyStateError} from "../components/emptyState.js";
```

# DMS Processing Dashboard

<div class="fade-in">

Monitor document processing in real-time. Track progress, view statistics, and identify issues.
...
if (requestId) {
  try {
    ...
    async function* autoRefreshStatus(requestId) {
      ...
      while (true) {
        const statusData = await dmsProcessingStatus(requestId);
...
const docStatusData = status && status.documents ? [
...
Plot.plot({
  ...
  y: {label: "Status"},
  x: {label: "Count"},
  color: {domain: docStatusData.map(d => d.status), range: docStatusData.map(d => d.color)},
  marks: [
    Plot.barX(docStatusData, {
      x: "count",
      y: "status",
      fill: "color"
    })
