---
title: Relational Processing Dashboard
description: Real-time processing status, progress, and statistics
---

```js
import * as Plot from "@observablehq/plot";
import relationalProcessingStatus from "../data/loaders/relational-processing.js";
import {html} from "@observablehq/stdlib";
import {emptyStateNoRequest, emptyStateLoading, emptyStateError} from "../components/emptyState.js";
```

# Relational Processing Dashboard

<div class="fade-in">

Monitor table processing in real-time. Track progress, view statistics, and identify issues.
...
if (requestId) {
  try {
    ...
    async function* autoRefreshStatus(requestId) {
      ...
      while (true) {
        const statusData = await relationalProcessingStatus(requestId);
  ...
  ${statusError ? emptyStateError(statusError) : status ? html`
    ...
    <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; margin-top: 24px;">
      <div style="text-align: center;">
        ...
        <div style="font-size: 14px; color: #86868b;">Tables Processed</div>
      </div>
      <div style="text-align: center;">
...
const tableStatusData = status && status.documents ? [
...
Plot.plot({
  ...
  y: {label: "Status"},
  x: {label: "Count"},
  color: {domain: tableStatusData.map(d => d.status), range: tableStatusData.map(d => d.color)},
  marks: [
    Plot.barX(tableStatusData, {
      x: "count",
      y: "status",
      fill: "color"
    })
