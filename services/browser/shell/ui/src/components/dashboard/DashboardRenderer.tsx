...
export function DashboardRenderer({ 
  ...
  const renderAdvancedChart = (chart: DashboardChart, index: number) => {
    ...
    if (chartType === 'sankey') {
      ...
      const sankeyData: { nodes: readonly any[], links: readonly any[] } = (chart.config?.sankey as any) || (chartData.length > 0 ? {
        nodes: chartData.map((item: any, idx: number) => ({ id: item.id || `node-${idx}`, nodeColor: "hsl(205, 70%, 50%)" })),
        links: chartData.map((item: any, idx: number) => ({ source: item.source || chartData[0]?.id || 'node-0', target: item.target || item.id || `node-${idx}`, value: item.value || 1 }))
      } : { nodes: [], links: [] });
