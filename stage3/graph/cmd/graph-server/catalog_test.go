...
func TestAgentCatalogHandlerIncludesEnrichment(t *testing.T) {
	...
	handler := newAgentCatalogHandler(server.Addr().String())
	...
	if summary, ok := payload["agent_catalog_summary"].(string); !ok || !strings.Contains(summary, "suite") {
	...
	tools, ok := payload["agent_catalog_unique_tools"].([]any)
	if !ok || len(tools) == 0 {
		t.Fatalf("expected agent_catalog_unique_tools array, got %T (%v)", payload["agent_catalog_unique_tools"], payload["agent_catalog_unique_tools"])
	}
}
