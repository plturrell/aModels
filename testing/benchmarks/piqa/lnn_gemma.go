...
func NewGemmaVaultCalibrator(cfg lnn.Config) (lnn.Calibrator, error) {
	...
	gemmaCfg := lnn.Config{
		...
		OutputSize:   12,     // Learn 12 parameters (embedding_dim, max_phrase_len, etc.)
...
func init() {
	...
	lnn.RegisterCalibrator("piqa-gemmavault", func(cfg lnn.Config) (lnn.Calibrator, error) {
		return NewGemmaVaultCalibrator(cfg)
	})

	// Also register with common model name variants
	lnn.RegisterCalibrator("piqa-gemma", func(cfg lnn.Config) (lnn.Calibrator, error) {
		return NewGemmaVaultCalibrator(cfg)
	})
}
