...
func NewModelAwareCalibrator(taskID string, cfg lnn.Config) (lnn.Calibrator, error) {
	// Extract model type from task ID if present (e.g., "piqa-gemmavault")
...
func init() {
	// Register the default PIQA calibrator with model awareness
	lnn.RegisterCalibrator("piqa", func(cfg lnn.Config) (lnn.Calibrator, error) {
		return NewModelAwareCalibrator("piqa", cfg)
	})
}
